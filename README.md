Campsite Commander
This app was desigened to store camping gear and food supplies
The app uses user input,parallel arrays, screen navigation and loops to manage the packing list
what is in the app
1 Slash screen
2 Main screen
3 Detailed view screen
4 Data storage
[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/3HAo3iZi)
